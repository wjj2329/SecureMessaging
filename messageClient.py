...
class Client:
    ...
    def parse_command(self,command):
        ...
        if fields[0] == 'read':
            ...
            return True
        if fields[0] == 'peek':
            ### read message without unencrypting ###
            try:
                name = fields[1]
                index = int(fields[2])
            except:
                return False
            self.send_read(name, index)
            self.response_to_read(decrypt=False)
            return True
        if fields[0] == 'login':
            ### log in ###
            try:
    ...
    def login(self, user):
        ...
        self.key = RSA.generate(self.key_size, rand)
        public_key = self.key.publickey()
        export = public_key.exportKey()
        request="store_key "+user+" "+str(len(export))+"\n"+export
    ...
    def read_key(self):
        ...
        if key!="key":
            ...
            return
        length=int(fields[1])
        key_string = self.read_get_key_response(length)
        return RSA.importKey(key_string)

    def read_get_key_response(self,length):
        data = self.cache
    ...
    def response_to_read(self, decrypt=True):
        message = self.get_response()
        fields = message.split()
        try:
            if fields[0] != 'message':
            ...
            print "Server returned bad message:",message
            return
        self.read_message_response(subject,length, decrypt=decrypt)

    def read_message_response(self,subject,length,decrypt=True):
        data = self.cache
        while len(data) < length:
            d = self.server.recv(self.size)
            if not d:
